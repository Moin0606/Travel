  ...
  useEffect(() => {
    const fetchOtherUserStatuses = async () => {
      ...
      for (const match of matches) {
        try {
          const status = await useMatchStore.getState().getOtherUserMatchStatus(
            match.postId._id
          );
          statuses[match.postId._id] = status;
        } catch (error) {
      ...
      {matches.map((match) => (
        ...
        >
          ...
          <p className="text-sm text-gray-700">
            Matched with User:{" "}
            {otherUserStatuses[match.userId.username]}
            Other User's Status:{" "}
            {otherUserStatuses[match.postId._id] || "Loading..."}
          </p>
          <p className="text-sm text-gray-700">
            Travel Dates:{" "}
